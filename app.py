...
def load_model(path):
    ...
    model.load_state_dict(torch.load(path, map_location=config.device, weights_only=False))
    model.eval()
    return model
...
# Streamlit UI
st.title("C++ code translator")
mode = st.radio("Select Translation Mode", ("C++ to Pseudocode", "Pseudocode to C++"))
